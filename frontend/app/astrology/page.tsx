...
export default function AstrologyPage() {
  ...
  const loadHoroscope = async (sign: string) => {
    setLoading(true);
    try {
      const h = await astrology.horoscope(sign);
      setHoroscope(h);
    } finally {
      setLoading(false);
    }
  ...
  const loadChart = async () => {
    setLoading(true);
    try {
      const c = await astrology.myChart();
      setChart(c);
    } catch {
      // Mock chart for demo
      setChart({
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle>{horoscope.sign} — {horoscope.date}</CardTitle>
                    <div className="flex gap-2">
                      <Badge variant="amber">#{horoscope.lucky_number}</Badge>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-white/30 text-xs mt-2">Update your profile with birth details for accuracy</p>
            </div>
          )}
        </div>
      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {signs.map((s) => (
                <Card key={s.sign}>
                  ...
                  <CardContent>
                    <p className="text-white/50 text-xs mb-1">Body: {s.body_area}</p>
